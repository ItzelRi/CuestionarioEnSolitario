...
import { IQuestion, IQuestionnaire } from "../GrobalTypes";
    ...
    try {
        ...
        for (const question of body.questions) {
            const objQuestion = {
                ...
                qstId: createdQuestionnaire._id
            };
...
export const getQuestionnaires = async (req: Request, res: Response): Promise<void> => {
  try {
    const questionnaire = await QuestionnaireModel.find();
    res.status(200).json({msg:"Cuestionarios obtenidos con exito", questionnaire})
    return;
  } catch (error) {
    console.log(error);
